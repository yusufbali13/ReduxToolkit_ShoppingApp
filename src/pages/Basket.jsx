...
export const Basket = () => {
  ...
  const calcTotalPrice = () => {
    basketItem.map((item) => (sumPrice += item.itemPrice * item.itemQuantity));
  };

  const calcTotalQuantity = () => {
    basketItem.map((item) => (sumQuantity += item.itemQuantity));
  };
    ...
    <div className="relative overflow-x-auto my-10 w-[50%] m-auto">
      ...
      <div className="p-3">
        <button
          ...
          className="bg-gray-300 p-1 rounded-md w-[100px] hover:bg-gray-400 hover:text-white"
        >
          Clear All
        </button>
      </div>
        ...
        {basketItem?.map((item) => (
          <tbody>
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
              ...
              <td className="px-6 py-4 text-center">{item.itemQuantity}</td>
